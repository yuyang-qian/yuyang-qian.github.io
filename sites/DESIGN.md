...
On wide screens, embedding aligns the greeting with the host's section headings. Affiliations, the introduction, and the Supervisor paragraph share a 24px text indent. Research categories use native square list markers with the same 40px list indentation and 8px item padding as the paper entries. The iframe extends into the host gutters (16px, or 10px at widths of 700px and below), keeping the card border outside the aligned text. The iframe spans the host's full width including these gutters, so both documents cross the 700px breakpoint together. The host retains a 520px minimum width even on narrow screens. At 700px and below, both card edges align with the host section headings and divider lines, using the 10px host gutter as the card inset and 4px of inner padding to keep the content in place. From 701px through 800px, the card uses 8px of horizontal inner padding. The affiliation row begins at the same left edge as the introduction, aligned with the host’s Professor paragraph, and the native square markers align with the host’s paper-list markers. Their offsets account for the host gutter, card inset, border, and inner padding. Keep the host gutter values and the `.embedded` CSS rules in sync.
