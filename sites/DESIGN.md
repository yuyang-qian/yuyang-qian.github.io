...
The dynamic card starts with a greeting heading and affiliations, followed by the research introduction and project groups. It has no theme control or contact footer. `../index_backup.html` embeds the card, which reports its height to the parent. Projects with a `paper_anchor` smoothly scroll to the matching paper in the current parent document, then update the URL fragment, focus the item, and play a single 1.6-second `#f6f7fa` → `#d4e0f0` → `#f6f7fa` background fade without an outline. Repeated clicks restart the fade. Native links to `../../index_backup.html#paper-…` also work outside the iframe. Projects without a paper anchor, such as PD Survey, retain their external URLs and open in new tabs when embedded. TreeLoRA follows OnlineRLHF. The introduction highlights “Efficiency” and “Infra”. The background is near-white (`#fdfeff`), and square-grid and pointer-wave opacity are scaled to 60% of the original values.
...
On screens of 700px or less, the affiliation row keeps both full labels, logos, and the slash on one line. Its font size adapts to the available width, with logo heights scaling proportionally. Other text keeps its existing font size. At widths of 800px or less, projects move below their category heading and use 32px of left padding. Each separator belongs to the preceding project entry, so wrapping never leaves a middle dot at the start of a line. The last project has no trailing separator.
...
Embedding aligns the greeting with the host's section headings. Affiliations, the introduction, and the Supervisor paragraph share a 24px text indent. Research categories use native square list markers with the same 40px list indentation and 8px item padding as the paper entries. The iframe extends into the host gutters (16px, or 10px at widths of 700px and below), keeping the card border outside the aligned text. The iframe spans the host's full width including these gutters, so both documents cross the 700px breakpoint together. The host retains a 520px minimum width even on narrow screens. The card has 6px of outside space on each side within that layout; its inner padding compensates for that inset to preserve text alignment. Keep the host gutter values and the `.embedded` CSS rules in sync.
