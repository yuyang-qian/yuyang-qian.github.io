...
The dynamic card starts with a greeting heading and affiliations, followed by the research introduction and project groups. It has no theme control or contact footer. `../index_backup.html` embeds the card, which reports its height to the parent. Projects with a `paper_anchor` navigate to the matching paper in the current parent document, update the URL fragment, focus the item, and outline it. Native links to `../../index_backup.html#paper-…` also work outside the iframe. Projects without a paper anchor, such as PD Survey, retain their external URLs and open in new tabs when embedded. TreeLoRA follows OnlineRLHF. The introduction highlights “Efficiency” and “Infra”. The background is near-white (`#fdfeff`), and square-grid and pointer-wave opacity are scaled to 60% of the original values.
